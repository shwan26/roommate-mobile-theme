...
function rmt_get_chat_url( $author_id, $post_id ) {
    return add_query_arg(
        array(
            'recipient' => absint($author_id),
            'listing'   => absint($post_id),
        ),
        home_url('/messages/')
    );
}
 ...
 * Requires a WordPress page with slug: messages
 * Assign template: Messages
 * ------------------------------------------------------------
 */
function rmt_messages_table_name() {
    global $wpdb;
...
function rmt_create_messages_table() {
    ...
    $table_name      = rmt_messages_table_name();
...
add_action('after_switch_theme', 'rmt_create_messages_table');

/**
 * Keep the messages table updated.
 * Version 2 adds read_at, so old sites get the new column automatically.
 */
function rmt_maybe_create_messages_table() {
    $current_version = (int) get_option('rmt_messages_table_version', 0);

    if ($current_version < 2) {
        rmt_create_messages_table();
        update_option('rmt_messages_table_created', 1);
        update_option('rmt_messages_table_version', 2);
    }
}
...
function rmt_user_can_chat_about_listing($user_id, $other_user_id, $listing_id) {
    ...
    if ($listing->post_status !== 'publish' && (int) $listing->post_author !== $user_id && (int) $listing->post_author !== $other_user_id) {
        return false;
    }

    return (int) $listing->post_author === $other_user_id || (int) $listing->post_author === $user_id;
}

function rmt_insert_message($sender_id, $recipient_id, $listing_id, $message) {
    global $wpdb;

    $message = trim(wp_strip_all_tags($message));

    if ($message === '') {
        return new WP_Error('empty_message', 'Please write a message.');
    }

    if (mb_strlen($message) > 1500) {
        return new WP_Error('message_too_long', 'Message is too long. Please keep it under 1500 characters.');
    }

    if (!rmt_user_can_chat_about_listing($sender_id, $recipient_id, $listing_id)) {
        return new WP_Error('not_allowed', 'You cannot send a message for this listing.');
    }

    $inserted = $wpdb->insert(
        rmt_messages_table_name(),
        array(
            'listing_id'    => absint($listing_id),
...
function rmt_mark_conversation_read($user_id, $other_user_id, $listing_id) {
    global $wpdb;
    ...
    $wpdb->update(
        ...
        array(
            ...
            'sender_id'    => absint($other_user_id),
            'listing_id'   => absint($listing_id),
        ),
        array('%d', '%s'),
        array('%d', '%d', '%d')
...
/**
 * Return the small status text shown below each message.
 * - Your outgoing message: Sent until the recipient opens it, then Read.
 * - Incoming message: Unread before it is marked read, then Read.
 */
function rmt_get_message_status_label($message, $current_user_id) {
    $current_user_id = absint($current_user_id);
    $is_mine         = (int) $message->sender_id === $current_user_id;
    $is_read         = !empty($message->is_read);

    if ($is_mine) {
        return $is_read ? 'Read' : 'Sent';
    }
    ...
    return $wpdb->get_results(
        $wpdb->prepare(
            "SELECT
                ...
                SUBSTRING_INDEX(
                    GROUP_CONCAT(m.message ORDER BY m.created_at DESC SEPARATOR '|||RMT|||'),
                    '|||RMT|||',
                    1
                ) AS last_message
             FROM {$table} m
